...
function updateSummary() {
  if (summaryRoom) summaryRoom.textContent = formatRoomLabel();
  if (summaryDate) summaryDate.textContent = formatDate(dateInput ? dateInput.value : '');
  if (summaryTime) summaryTime.textContent = formatTimeRange(startTimeInput ? startTimeInput.value : '', endTimeInput ? endTimeInput.value : '');
  if (summaryDuration) summaryDuration.textContent = calculateDuration(startTimeInput ? startTimeInput.value : '', endTimeInput ? endTimeInput.value : '');
  if (summaryPurpose) summaryPurpose.textContent = (purposeInput ? purposeInput.value.trim() : '') || '-';
  if (summaryParticipants) summaryParticipants.textContent = (participantInput ? participantInput.value : '') || '-';
  if (summaryRequests) summaryRequests.textContent = (specialRequestsInput ? specialRequestsInput.value.trim() : '') || '-';
}
...
function resetSummary() {
  ...
  if (summaryPurpose) summaryPurpose.textContent = '-';
  if (summaryParticipants) summaryParticipants.textContent = '-';
  if (summaryRequests) summaryRequests.textContent = '-';
}
